...
for ((i=START_INDEX; i<START_INDEX+N; i++)); do
  ...
  python train.py task=AnymalTerrain_NeuroRL_train \
    ...
    task.env.urdfAsset.file=urdf/anymal_c/urdf/anymal_minimal_0pt5MASS.urdf \
    task.env.urdfAsset.mass=26.58213 \
    task.env.learn.perturbRandom.perturbRandomOn=false \
    task.env.learn.perturbRandom.forceX=0 \
    task.env.learn.perturbRandom.forceY=0 \
    task.env.learn.perturbRandom.forceZ=0 \
